...
namespace ConsoleApplication21
{
    ...
    { 
        ...
        IDictionary<string, IList<string>> ParseCommands(string[] args)
        {
            ...
            // once parced , Lets check for the correct use of the commands
            if (commands.ContainsKey("map") && commands["map"].Count == 0)
            {
                throw new Exception("command -k must have lust one parametr");
            }
        ...
        void Ping()
        {
            Console.WriteLine("Pingping.....");
            Console.Beep();
        }
        void Print (string[] masseges)
        {
            Console.WriteLine(String.Join(" ", masseges));
        }
